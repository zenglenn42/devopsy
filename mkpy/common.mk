#============================================================================
#
# Common make variables and rules.
#
...
#
#    make installhere    installs here:   ./here/opt/devopsy/*
# 
# Alternatively,
#
#    sudo make install   installs here:   /opt/devopsy/*
#
# Optionally,
#
#    export DESTDIR=/var/tmp
#    sudo make install   installs here:   /var/tmp/opt/devopsy/*
#
...
# Define a package-relative conf dir (e.g., /opt/devopsy/etc).  This collapses
...
#     Use 'pkgconfdir' for stuff that goes in /opt/devopsy/etc
...
ifndef CHECK_TOOLS
    ...
    CHECK_TOOLS  = $(AWK)    $(CAT)   $(CURL)  $(CUT)    $(CHOWN)  $(DATE)  $(EGREP)
    CHECK_TOOLS += $(FIND)   $(GCC)   $(HEAD)   $(ID)     $(JAVA)  $(LN)
    CHECK_TOOLS += $(MD5SUM) $(MKDIR) $(MVN)    $(PKGCONFIG)       $(OBJDUMP) $(OTOOL)
    CHECK_TOOLS += $(PIP)    $(PR)    $(PYTHON) $(RM)    $(READELF)
...
REQD_PYTHON_MIN_VER = 2.7.16
# e.g., Convert 2.7.2 -> 2.7
...
ifndef DO_BUILD_VERBOSE
    DO_BUILD_VERBOSE = 0
endif

ifeq ($(DO_BUILD_VERBOSE),1)
    quiet =
    Q =
else
    quiet=quiet_
...
#----------------------------------------------------------------------------
# List shared lib deps
#----------------------------------------------------------------------------
ifeq "$(DISTRO)" "Darwin"
define lsshlib-deps
        ($(OBJDUMP) -p $1 | grep name | grep -v sectname | grep -v segname | awk '{print $$2}') | tee $2
endef
else
define lsshlib-deps
        ($(READELF) --dynamic $1 | grep NEEDED | sed -e "s|.*\(NEEDED.*\)|\1|g" -e "s|)||g" -e "s| [ ]*| |g" ) | tee $2
endef
endif
#----------------------------------------------------------------------------
quiet_cmd_LSSHLIBS = LSSHLIBS   $2
      cmd_LSSHLIBS = $(call lsshlib-deps,$2,$3)

#----------------------------------------------------------------------------
# List shared lib deps
#----------------------------------------------------------------------------
# Here, we alias a failing grep match to nominal since no match is okay
# (Otherwise the build will assume this is an error because grep returns -1.)
ifeq "$(DISTRO)" "Darwin"
define lsrpaths
        ($(OTOOL) -L $1 | awk '{print $$1}' |grep "@rpath" || exit 0) | tee $2
endef
else
define lsrpaths
        ($(READELF) --dynamic $1 | egrep "RPATH|RUNPATH" || exit 0) | tee $2
endef
endif
#----------------------------------------------------------------------------
quiet_cmd_LSRPATHS = LSRPATHS   $2
      cmd_LSRPATHS = $(call lsrpaths,$2,$3)
...
dflt_component_help:
	@echo
	@echo "DevOpsy $(_COMPONENT) makefile"
	@echo
...
dflt_component_mrclean dflt_component_distclean: dflt_component_clean
	@for target_file in $(wildcard $(CHECKED_TOOLS_VERSION_BRAND) $(CHECKED_TOOLS) $(CHECKED_ENV) $(CHECKED_SHLIB_DEPS) $(CHECKED_RPATHS)) ;\
	do \
